...
!(function (win, doc, nav, top, useAtomics, config, libPath, timeout, scripts, sandbox, mainForwardFn, isReady) {
      ...
      if (libPath[0] == '/') {
        ...
        if (top != win) {
          ...
          useAtomics
            ? loadSandbox(1)
            : nav.serviceWorker
              ? nav.serviceWorker.register(libPath + (config.swPath || 'partytown-sw.js'), {
                scope: libPath
  ...
  top == win && (config.forward || []).map((function (forwardProps) {
    ...
    forwardProps.split('.').map((function (_, i, forwardPropsArr) {
      mainForwardFn = mainForwardFn[forwardPropsArr[i]] = i + 1 < forwardPropsArr.length
        ? forwardPropsArr[i + 1] == 'push' ? [] : mainForwardFn[forwardPropsArr[i]] || {}
        : function () {
