  ...
  const initCanvas = (WorkerBase, win) => {
    ...
    const createContext2D = (canvasInstance, contextType, contextAttributes) => {
      ...
      const CanvasRenderingContext2D = {
        get: (target, propName) => typeof propName === 'string' && propName in ctx
        ...
        set (target, propName, value) {
          if (typeof propName === 'string' && propName in ctx) {
            ctx[propName] !== value && typeof value !== 'function' && setter(ctxInstance, [propName], value)
            ctx[propName] = value
          } else {
    ...
    const createContextWebGL = (canvasInstance, contextType, contextAttributes) => {
      ...
      const WebGLRenderingContextHandler = {
        get: (target, propName) => typeof propName === 'string' ? typeof ctx[propName] !== 'function' ? ctx[propName] : (...args) => callMethod(ctxInstance, [propName], args, getWebGlMethodCallType(propName)) : target[propName],
        set (target, propName, value) {
          if (typeof propName === 'string' && propName in ctx) {
            ctx[propName] !== value && typeof value !== 'function' && setter(ctxInstance, [propName], value)
            ctx[propName] = value
    ...
    const WorkerAudioTrackList = class {
      constructor (mediaElm) {
        ...
        return new Proxy(instance, {
          get: (target, propName) => typeof propName === 'number' ? new WorkerAudioTrack(winId, instanceId, ['audioTracks', propName]) : target[propName]
        })
      }
    }
